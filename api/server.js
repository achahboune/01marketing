...
app.post('/send-email', async (req, res) => {
  ...
  const visitorName = name?.trim() || "Visiteur";
  ...
  try {
    ...
    // Debug des réponses Brevo
    const adminJson = await resAdmin.json();
    const userJson = await resUser.json();
    console.log('Réponse Brevo Admin:', adminJson);
    console.log('Réponse Brevo Visiteur:', userJson);

    if (!resAdmin.ok || !resUser.ok) {
      return res.status(500).json({
        error: "Erreur API Brevo",
        details: { admin: adminJson, user: userJson }
      });
    }
